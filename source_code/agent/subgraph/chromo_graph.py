...
from source_code.agent.tools.chromo_tools import remove_background_of_original_image, segment_mid_image, recognize_image
from source_code.agent.tools.chromo_cls_predict import predictor
...
class AskAiForChromoCls(BaseModel):
    """向AI模型求助，获取图像的分类结果"""
...
async def planer_node(state: State) -> Command[Literal["ask_ai", "plan_node", "__end__"]]:
    ...
    if message.tool_calls:
        if message.tool_calls[0]["name"] == "AskAiForChromoCls":
            return Command(
                goto="ask_ai",
...
def ask_ai_node(state: State) -> Command[Literal["planer"]]:
    tool_call_id = state["messages"][-1].tool_calls[0]["id"]
    tool_name = state["messages"][-1].tool_calls[0]["name"]
    ask = AskAiForChromoCls.model_validate(state["messages"][-1].tool_calls[0]["args"])
    pred_class, _ = predictor.predict_image(ask.img_path)
    return Command(
        goto="planer",
        update={
            "messages": [
                ToolMessage(content=f"这张图像的类型是{pred_class}", tool_call_id=tool_call_id, name=tool_name)
            ]
        },
    )
...
workflow.add_node("ask_ai", ask_ai_node)
